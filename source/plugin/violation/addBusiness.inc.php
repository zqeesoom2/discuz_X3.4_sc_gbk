...
class Violation{
	public function __construct() {
		...
		$this->Config['PluginVar'] = (array) $_G['cache']['plugin']['violation'];//获取标识符为violation的插件系统设置的变量

		$this->Config['LangVar'] = lang('plugin/violation'); //把定义的脚本插件语言导入进来
...
if(!submitcheck('DetailSubmit')) {
    ...
    /* $Query = C::t('common_usergroup')->range_orderby_credit();
     $UserGroupList[0] = cplang('plugins_empty'); //加载语言包,这里是空

     foreach($Query as $Id => $UserGroup) {
         $UserGroupList[$UserGroup['groupid']] = $UserGroup['grouptitle']; //得到所有的用户组
     }

     foreach($_G['setting']['extcredits'] as $Id => $Credit) {
         $ExtcreditsList[$Id] = $Credit['title'];//威望\金钱\贡献
     }*/

    $formUrl = ltrim(rawurldecode(cpurl()),'action='); //form表单
    showformheader($formUrl,'enctype');
    showtableheader(); //表头
    showtitle($OpTitle);

    showsetting('商家UID', 'uid', $Item['param']['uid'], 'text');

    //               标题名          指定表单的name值        值，           表单类型
    showsetting('商家名称', 'business', $Item['business'], 'text');
    if($Item['param']['logo']) {//有没有获得图片
        $BannerHtml = '<label><input type="checkbox" class="checkbox" name="dellogo" value="yes" /> 删除</label><br /><a href="'.$Item['param']['logo'].'" target="_blank"><img src="'.$Item['param']['logo'].'" width="100"/></a><input type="hidden" value="'.$Item['param']['logo'].'" name="logo"><br />建议640 * 320';
    }else{
        //$BannerHtml = $Violation->Config['LangVar']['BannerPrompt'];//建议640 * 320
        $BannerHtml = '建议640 * 320';
    }
    //                                                                         是否能修改，   是否隐藏  ，强制描述符
    showsetting('商家logo', 'logonew','', 'filetext', '', 0, $BannerHtml);

    showsetting('开始时间', 'start_dateline',$Item['start_dateline'] ? date('Y-m-d H:i',$Item['start_dateline']):'', 'calendar','','','',1);

    showsetting('结束时间', 'end_dateline',$Item['end_dateline']?date('Y-m-d H:i',$Item['end_dateline']):'', 'calendar','','','',1);


    showsetting('海报内容', 'postercontent', $Item['param']['postercontent'] ? $Item['param']['postercontent'] : '我在{AppName}抢到了{br}{em}{Money}{em}元', 'textarea','','','{AppName}代表APP名字，{UserName}代表当前活动的用户，{br}代表换行，{em}代表你要加粗加颜色的字{/em}');

    //商家介绍
    echo '<tr><td colspan="2" class="td27" s="1">商家介绍</td></tr><tr class="noborder"><td class="vtop rowform" colspan="2"><textarea id="business" name="businessinfo" style="width:80%;height:300px;visibility:hidden;">'.stripslashes($Item['param']['businessinfo']).'</textarea></td></tr>';


    showsetting('分享标题', 'sharetitle', $Item['param']['sharetitle'], 'text','', 0, '14个字以内');
    showsetting('分享描述', 'sharedesc', $Item['param']['sharedesc'], 'textarea','', 0, '20个字以内');


    $ShareLogoHtml = '<label><input type="checkbox" class="checkbox" name="delposter" value="yes" /> 删除</label><br /><a href="'.$Item['param']['poster'].'" target="_blank"><img src="'.$Item['param']['poster'].'" width="100"/></a><input type="hidden" value="'.$Item['param']['poster'].'" name="poster"><br />建议尺寸640*1065';

    showsetting('海报图片', 'posternew','', 'filetext', '', 0, $ShareLogoHtml);

    showsetting('显示顺序', 'displayorder', $Item['displayorder'], 'text');

    showsetting('添加时间', 'dateline',date('Y-m-d H:i',$Item['dateline'] ? $Item['dateline'] : time()), 'calendar','','','',1);
    ...
    if (submitcheck('formhash')) {
        $Data['business'] = addslashes(strip_tags($_GET['business']));//剥去字符串中的 HTML 标签,在预定义字符[单引号，双引号，NULL,反斜杠]之前添加反斜杠的字符串
        ...
        $Param['businessinfo'] = addslashes($_GET['businessinfo']);//需保留Html

        $Param['postercontent'] = addslashes(strip_tags($_GET['postercontent']));//海报内容
        ...
        if ($_FILES['logonew']['size']) {
            ...
            if ($BannerFile['Errorcode']) {
                cpmsg('图片格式不正确，请上传jpg,jpeg,gif,png,bmp格式的图片', '', 'error');
            } else {
        ...
        memory('set', 'HbShareItem' . $AId, $ItemTo, 3600);//写入缓存
        cpmsg('更新成功', $CpMsgUrl, 'succeed');
    }
}
?>