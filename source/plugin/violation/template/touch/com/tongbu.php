...
if($config['es_fid']){
	if($config['isrewrite']){
		...
		$re_url='plugin.php?id=aljesc&act=view&lid='.$insertid;
	}
	$tongbu_zujin=intval($_GET['zujin'])>0?$_GET['zujin']:lang('plugin/aljesc','aljesc_23');
	...
	C::t('#aljesc#aljesc')->update($insertid,array('tid'=>$tid));
}
?>