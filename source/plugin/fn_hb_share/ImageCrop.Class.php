...
function RotateImage($File,$Orientation){
	...
	switch ($Type) {
		case IMAGETYPE_JPEG:
			$rImage = imagecreatefromjpeg($File);
		break;
		case IMAGETYPE_PNG:
			$rImage = imagecreatefrompng($File);
		break;
		case IMAGETYPE_GIF:
			$rImage = imagecreatefromgif($File);
		break;
		default:
		exit();
	}
	if(!empty($Orientation)){
		switch ($Orientation) {
			case 8:
				$rImage = imagerotate($rImage, 90, 0);
			break;
			case 3:
				$rImage = imagerotate($rImage, 180, 0);
			break;
			case 6:
				$rImage = imagerotate($rImage, -90, 0);
			break;
	...
	switch ($Type) {
		case IMAGETYPE_JPEG:
			imagejpeg($rImage,$File, 100);
		break;
		case IMAGETYPE_PNG:
			imagepng($rImage,$File);
		break;
		case IMAGETYPE_GIF:
			imagegif($rImage,$File);
		break;
		default:
		break;
	}
